...
export function resolveModifiers(state, atMs) {
  ...
  // A modifier may target one supply line. Women's Work boosts
  // clothing, not the kitchen — keyed separately so it can't leak.
  const addScoped = (mod, tree) => {
    if (mod.type === 'production_speed' && mod.supply) {
      const key = `production_speed:${mod.supply}`;
      scoped.add(key);
      trees[key] = trees[key] || {};
      trees[key][tree] = (trees[key][tree] || 0) + mod.value;
      return true;
    }
    return false;
  };
  ...
  for (const owned of state.ministries || []) {
    ...
    for (const mod of def.modifiers) {
      ...
      if (addScoped({ ...mod, value: mod.value * scale }, def.tree || def.id)) continue;
  ...
  for (const key of scoped) {
    let product = 1;
    for (const value of Object.values(trees[key] || {})) product *= 1 + value;
    out[key] = product;
  }
