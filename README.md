...
**Tests** (197 assertions, no browser required):
...
Away" card · named characters · sermon selection · seated figures actually sitting
in the pews · most ministries beyond their data entries · any audio · Supabase sync
is written but never called from `main.js`
...
**Pew orientation is load-bearing.** `pewLayout()` returns `chancelZ` and a
